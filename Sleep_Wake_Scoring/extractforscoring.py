...
def extract_lfp(filename_sw):
    ...
    if not cort:
        file_LFP_selected_channels = \
            op.join(op.join(LFP_dir, 'LFP_chancheck'),
                    'selected_channels.npy')
        if op.isfile(file_LFP_selected_channels):
            selected_chans = \
                np.load(file_LFP_selected_channels)
        else:
            raise \
                FileNotFoundError('Check manual section Find best channels' +
                                  '. Missing file ' +
                                  file_LFP_selected_channels)
    else:
    ...
    for fil in filesindex:
        ...
        # THIS CAN BE CONSOLIDATED INTO A FEW LINES LATER.
        # THIS IS WHEN SAM IS LEARNING...
        # start importing your data

        # select the 12 files that you will work on at a time.
        # this is to prevent
        # overloading RAM; since the default is to write 5 minute files,
        # this will
        ...
        for a in np.arange(0, np.size(load_files)):
            if not cort:
                ...
                t, dgc = \
                    ntk.load_raw_gain_chmap_1probe(load_files[a], numchan, HS,
                                                   nprobes, te=-1,
                                                   probenum=probenum,
                                                   probechans=64)
                ...
                full_selected_eeg = \
                    np.concatenate([full_selected_eeg, selected_eeg], axis=1)
        ...
        print('THIS IS THE USAGE AT THE END OF LOOP: ',
